    ...
    <div class="container">
      <div class="page-header">
        <el-breadcrumb separator="/">
          <el-breadcrumb-item><router-link to="/community">学习社区</router-link></el-breadcrumb-item>
      ...
      <div class="post-content" v-loading="loading">
        ...
        <div class="post-actions">
          <el-button type="primary" @click="likePost" :icon="isLiked ? 'StarFilled' : 'Star'" :loading="liking">
            {{ isLiked ? '已点赞' : '点赞' }}
          </el-button>
          <el-button @click="showCommentDialog = true" icon="ChatDotRound">
            评论
          </el-button>
        </div>
      </div>
            ...
            <div class="comment-content">
              ...
              <div class="comment-actions">
                <span @click="likeComment(comment.id)" class="action">
                  <el-icon :size="16">{{ comment.is_liked ? 'StarFilled' : 'Star' }}</el-icon>
                ...
                <span v-if="canModerate" @click="togglePinComment(comment)" class="action pin-action">
...
const fetchComments = async () => {
  ...
  try {
    console.log('获取评论，帖子ID:', postId.value)
    console.log('分页参数:', { page: commentPagination.page, limit: commentPagination.limit })
    ...
    console.log('处理后评论数:', comments.value.length)
  } catch (error) {
